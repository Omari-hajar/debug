...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        submitBtn.setOnClickListener {
            if(usernameAccepted(userName.text.toString()) && passwordAccepted(password.text.toString())){
                // we compare the two password values => don't add passwordAccepted since it return a boolean true it would not compare
                if (password.text.toString() == password2.text.toString()) {
                    Toast.makeText(this, "User created!", Toast.LENGTH_LONG).show()
                    users.add(userName.text.toString().capitalize())
                    displayUsers()

                    //make the container that hold the sign in invisiable
                    llMain.isVisible = false


                }else{
                    Toast.makeText(this, "Make sure the two passwords matches", Toast.LENGTH_LONG).show()
                }
    ...
    private fun hasUpper(text: String): Boolean{
        var letter = 'A'
        while (letter <= 'Z') {
            //check if capital letter in the whole input not only beginning
